...
mkdir -p "$SANDBOX/scripts" "$SANDBOX/dist/Codex TPS.app" "$FAKE_BIN"
...
APP_STAPLE="$(line_for "stapler staple $SANDBOX/dist/Codex TPS.app")"
APP_VALIDATE="$(line_for "stapler validate $SANDBOX/dist/Codex TPS.app")"
...
echo "Verified app-first notarization and dual stapling sequence."
