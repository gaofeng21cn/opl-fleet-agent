...
APP_PATH="${CODEX_TPS_APP_PATH:-$ROOT_DIR/dist/Codex TPS.app}"
...
APP_ZIP="$TEMP_ROOT/Codex-TPS-app.zip"
cleanup() {
  rm -rf "${TEMP_ROOT:?}"
}
...
submit_and_wait() {
  ...
  local notary_status="$(plutil -extract status raw -o - "$result_plist")"
